...
type PointerGrab = {
  ...
  deliberateRelease: boolean;
};

export class Input {
  mode: 'free' | 'hop' | 'preview' = 'free';
  onRelease: () => void = () => {};
  ...
  constructor(
    ...
    this.reset = reset;
    this.canvas = canvas;
    this.grabBVH = new SurfaceBVH(body.surface);
    this.controls = new OrbitControls(camera, canvas);
    const c = this.controls;
    c.target.copy(body.center);
    this.follow.copy(c.target);
    c.enablePan = false;
    ...
    if (this.joystickElement) {
      ...
      this.joystickElement.addEventListener('pointermove', this.joystickMove, {
        passive: false,
        signal,
      });
    ...
    for (const button of document.querySelectorAll<HTMLButtonElement>('[data-control]')) {
      button.addEventListener(
        ...
        (e) => {
          ...
          this.touchKeys.set(e.pointerId, code);
  ...
  private joystickStart = (e: PointerEvent) => {
    ...
    this.joystickElement?.setPointerCapture(e.pointerId);
    this.joystickElement?.classList.add('held');
    this.joystickMove(e);
  };
  private joystickMove = (e: PointerEvent) => {
    ...
    const rect = joystick.getBoundingClientRect(),
      knob = this.joystickKnob?.getBoundingClientRect();
    ...
    const offsetX = dx * scale,
      offsetY = dy * scale;
    this.joystickX = offsetX / radius;
    this.joystickZ = -offsetY / radius;
  ...
  private releaseJoystick = (e: PointerEvent) => {
    ...
    e.preventDefault();
    const joystick = this.joystickElement,
      id = this.joystickPointer;
    this.joystickPointer = null;
    this.joystickX = 0;
    this.joystickZ = 0;
    this.joystickKnob?.style.setProperty('--joystick-x', '0px');
    this.joystickKnob?.style.setProperty('--joystick-y', '0px');
  ...
  private begin = (e: PointerEvent) => {
    if (this.mode === 'preview' || (this.mode === 'hop' && this.body.grab)) return;
    if (e.button !== 0 || this.grabs.has(e.pointerId) || this.body.grabs.length >= MAX_GRABS)
      return;
    // Only touch can add simultaneous grips; desktop mouse/pen keep one grip.
    if (
      this.body.grab &&
      (e.pointerType !== 'touch' ||
        [...this.grabs.values()].some((state) => state.pointerType !== 'touch'))
    )
      return;
    this.eventRay(e);
    ...
    if (this.body.gpuSurface) this.body.updateSurface(true);
    this.grabBVH.refit();
    ...
    const grab = surfaceGrab(this.body, face, point);
    if (!grab) return;
    this.body.grabs.push(grab);
    this.body.wake();
  ...
  private pointerMove = (e: PointerEvent) => {
    ...
    if (state && !state.releasePending) {
      ...
      e.preventDefault();
      e.stopImmediatePropagation();
      this.captureDragTarget(e, state);
      ...
      this.canvas.style.cursor =
        ...
        this.raycaster.ray.intersectsBox(this.mesh.geometry.boundingBox)
          ? 'grab'
          : 'default';
    }
  };
  private end = (e: PointerEvent) => {
    ...
    state.deliberateRelease = e.type === 'pointerup';
  ...
  private syncGrabControls() {
    this.controls.enabled = this.mode === 'free' && this.body.grabs.length === 0;
    this.canvas.classList.toggle(
      'grabbing',
      [...this.grabs.values()].some((state) => !state.releasePending),
    );
  }
  private finishRelease = (id?: number) => {
    const ids = id === undefined ? [...this.grabs.keys()] : [id];
    for (const pointerId of ids) {
      const state = this.grabs.get(pointerId);
      if (!state) continue;
  ...
  private keyDown = (e: KeyboardEvent) => {
    ...
    if (this.mode !== 'free') {
      if (e.code === 'KeyR' && !e.repeat && this.mode === 'hop') this.reset();
      return;
    }
    if (
      [
  ...
  step(h: number) {
    let x =
      Number(this.pressed('KeyD', 'ArrowRight')) -
      Number(this.pressed('KeyA', 'ArrowLeft')) +
      this.joystickX;
    let z =
      Number(this.pressed('KeyW', 'ArrowUp')) -
      Number(this.pressed('KeyS', 'ArrowDown')) +
      this.joystickZ;
    ...
    for (const state of this.grabs.values()) {
      ...
      state.consumedVersion = state.commandVersion;
      state.physicsSteps++;
    }
  }
  ...
  afterPhysicsStep() {
    for (const [id, state] of this.grabs) {
      if (state.releasePending && state.consumedVersion === state.commandVersion) {
        ...
        if (state.releaseStepsRemaining <= 0) {
          const count = state.deliberateRelease;
          this.finishRelease(id);
          if (count) this.onRelease();
        }
      }
    }
  }
  update(dt: number) {
    ...
    for (const [id, state] of this.grabs)
      if (!this.body.grabs.includes(state.grab)) this.finishRelease(id);
    if (this.mode !== 'free') return;
    if (this.body.grab) return; // Freeze both orbit and translation for the entire grab.
    const target = this.temp.copy(this.body.center);
  ...
  setMode(mode: 'free' | 'hop' | 'preview') {
    this.clear();
    this.mode = mode;
    this.syncGrabControls();
    // Flush residual orbit damping before the fixed gameplay pose is installed.
