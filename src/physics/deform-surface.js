 ...
 */
export function deformSurface(surface, x, nodalF, center = null, vertex = -1) {
  const p = surface.positions,
    n = surface.geometry.attributes.normal.array;
  const ids = surface.bindingIds,
    w = surface.bindingWeights,
    rest = surface.restNormals;
  let minX = Infinity,
    minY = Infinity,
    minZ = Infinity,
    maxX = -Infinity,
  ...
  const cx = center?.x ?? 0,
    cy = center?.y ?? 0,
    cz = center?.z ?? 0;
  const first = vertex < 0 ? 0 : vertex,
    last = vertex < 0 ? p.length / 3 : vertex + 1;
  for (let i = first * 3, j = first * 4; i < last * 3; i += 3, j += 4) {
    const i0 = ids[j],
      i1 = ids[j + 1],
      i2 = ids[j + 2],
      i3 = ids[j + 3];
  ...
  if (center && geometry.boundingBox && geometry.boundingSphere) {
    ...
    geometry.boundingSphere.radius = Math.sqrt(maxR2);
  } else {
