...
export const STUDIO_KEY = new Vector3(-0.58, 0.74, -0.34).normalize();
...
export function shapeStudioLight(image: HDRImage, originalWindow: Vector3): HDRImage {
  ...
  const core = Math.cos(0.22),
    edge = Math.cos(0.7);
  for (let y = 0; y < height; y++)
    for (let x = 0; x < width; x++) {
      const phi = ((y + 0.5) / height - 0.5) * Math.PI,
        theta = ((x + 0.5) / width - 0.5) * Math.PI * 2;
      direction
        .set(Math.cos(phi) * Math.cos(theta), Math.sin(phi), Math.cos(phi) * Math.sin(theta))
        .applyQuaternion(rotation);
      const u = (Math.atan2(direction.z, direction.x) / (2 * Math.PI) + 0.5) * width - 0.5;
      const v = (Math.asin(Math.max(-1, Math.min(1, direction.y))) / Math.PI + 0.5) * height - 0.5;
      ...
      const gain = 0.14 + 2.86 * mask;
      const dst = (y * width + x) * 4;
      for (let c = 0; c < 3; c++) {
        let value = 0;
        for (let dy = 0; dy <= 1; dy++)
          for (let dx = 0; dx <= 1; dx++) {
            const sx = (((ix + dx) % width) + width) % width,
              sy = Math.max(0, Math.min(height - 1, iy + dy));
            value +=
              DataUtils.fromHalfFloat(data[(sy * width + sx) * channels + c]) *
              (dx ? fx : 1 - fx) *
              (dy ? fy : 1 - fy);
          }
