...
const IOR = 1.35;
const LIGHT_CAMERA_DISTANCE = 0.34;
const LIGHT_CAMERA_NEAR = 0.01;
const LIGHT_CAMERA_FAR = 0.72;

class SurfaceBVH {
  constructor(surface) {
    this.surface = surface;
    this.p = surface.positions;
    this.index = surface.indices;
    this.centroids = new Float32Array(this.index.length);
    for (let t = 0; t < this.index.length / 3; t++)
      for (let axis = 0; axis < 3; axis++) {
        this.centroids[t * 3 + axis] =
          (this.p[this.index[t * 3] * 3 + axis] +
            this.p[this.index[t * 3 + 1] * 3 + axis] +
            this.p[this.index[t * 3 + 2] * 3 + axis]) /
          3;
      }
    const build = (ids) => {
      ...
      if (ids.length <= 8) node.ids = ids;
        ...
        const ranges = [0, 1, 2].map((axis) => {
          let lo = Infinity,
            hi = -Infinity;
          for (const id of ids) {
            const x = this.centroids[id * 3 + axis];
            lo = Math.min(lo, x);
            hi = Math.max(hi, x);
          }
          return hi - lo;
        ...
        node.left = build(ids.slice(0, mid));
        node.right = build(ids.slice(mid));
      }
      return node;
    };
    this.root = build(Array.from({ length: this.index.length / 3 }, (_, i) => i));
    this.refit();
  }
  refit() {
    const p = this.p,
    ...
    const visit = (node) => {
      if (node.ids) {
        node.min.fill(Infinity);
        node.max.fill(-Infinity);
        for (const t of node.ids)
          for (let c = 0; c < 3; c++)
            for (let a = 0; a < 3; a++) {
              const v = p[ix[t * 3 + c] * 3 + a];
              node.min[a] = Math.min(node.min[a], v);
              node.max[a] = Math.max(node.max[a], v);
            }
      } else {
        visit(node.left);
        visit(node.right);
        for (let a = 0; a < 3; a++) {
          node.min[a] = Math.min(node.left.min[a], node.right.min[a]);
          node.max[a] = Math.max(node.left.max[a], node.right.max[a]);
        }
      }
    };
    visit(this.root);
  ...
  normal(hit, d, entering) {
    const ix = this.index,
      p = this.p,
      n = this.surface.geometry.attributes.normal.array;
    const a = ix[hit.t * 3] * 3,
      b = ix[hit.t * 3 + 1] * 3,
      c = ix[hit.t * 3 + 2] * 3,
      w = 1 - hit.u - hit.v;
    let x = n[a] * w + n[b] * hit.u + n[c] * hit.v,
      y = n[a + 1] * w + n[b + 1] * hit.u + n[c + 1] * hit.v,
      z = n[a + 2] * w + n[b + 2] * hit.u + n[c + 2] * hit.v;
    const ex = p[b] - p[a],
      ey = p[b + 1] - p[a + 1],
      ez = p[b + 2] - p[a + 2];
    const fx = p[c] - p[a],
    ...
    if ((x * d[0] + y * d[1] + z * d[2]) * sign > -0.015) {
      x = gx;
      y = gy;
      z = gz;
    }
...
function refractRay(d, n, n1, n2) {
  const cosine = clamp(-(d[0] * n[0] + d[1] * n[1] + d[2] * n[2]), 0, 1),
    eta = n1 / n2;
  ...
  return {
    direction: [eta * d[0] + a * n[0], eta * d[1] + a * n[1], eta * d[2] + a * n[2]],
    transmission: 1 - (rs * rs + rp * rp) / 2,
  };
}
...
function updateViewThickness(surface, bvh, camera) {
  const p = surface.positions,
    n = surface.geometry.attributes.normal.array;
  const thickness = surface.geometry.attributes.opticalThickness;
  for (let i = 0; i < p.length; i += 3) {
    let dx = p[i] - camera.position.x,
      dy = p[i + 1] - camera.position.y,
      dz = p[i + 2] - camera.position.z;
    const length = Math.hypot(dx, dy, dz) || 1;
    dx /= length;
    dy /= length;
    dz /= length;
    const normal = [n[i], n[i + 1], n[i + 2]];
    if (dx * normal[0] + dy * normal[1] + dz * normal[2] > -0.01) continue;
    const refraction = refractRay([dx, dy, dz], normal, 1, IOR);
    if (!refraction) continue;
    const dir = refraction.direction,
      o = [p[i] + dir[0] * 2e-6, p[i + 1] + dir[1] * 2e-6, p[i + 2] + dir[2] * 2e-6];
    const hit = bvh.hit(o, dir);
    thickness.array[i / 3] = hit ? clamp(hit.distance, 0.0002, 0.16) : 0.002;
  }
...
class OpticalShadowField {
  ...
  rasterTriangle(a, b, c, buffer, value) {
    const n = this.size,
      scale = n / this.span;
    const ax = (a[0] - this.origin.x) * scale,
      ay = (a[1] - this.origin.y) * scale;
    const bx = (b[0] - this.origin.x) * scale,
      by = (b[1] - this.origin.y) * scale;
    const cx = (c[0] - this.origin.x) * scale,
      cy = (c[1] - this.origin.y) * scale;
    const area = (bx - ax) * (cy - ay) - (by - ay) * (cx - ax);
    if (Math.abs(area) < 1e-9) return;
    const minX = clamp(Math.floor(Math.min(ax, bx, cx)), 0, n - 1),
      maxX = clamp(Math.ceil(Math.max(ax, bx, cx)), 0, n - 1);
    const minY = clamp(Math.floor(Math.min(ay, by, cy)), 0, n - 1),
      maxY = clamp(Math.ceil(Math.max(ay, by, cy)), 0, n - 1);
  ...
  blur(buffer) {
    const n = this.size,
      tmp = this.blurScratch;
    for (let y = 0; y < n; y++)
      for (let x = 0; x < n; x++) {
        let sum = 0;
        for (let k = -2; k <= 2; k++)
          sum += buffer[y * n + clamp(x + k, 0, n - 1)] * (3 - Math.abs(k));
        tmp[y * n + x] = sum / 9;
      }
    for (let y = 0; y < n; y++)
      for (let x = 0; x < n; x++) {
        let sum = 0;
        for (let k = -2; k <= 2; k++)
          sum += tmp[clamp(y + k, 0, n - 1) * n + x] * (3 - Math.abs(k));
        buffer[y * n + x] = sum / 9;
      }
  }
  update(body) {
    ...
    const p = this.surface.positions,
      ix = this.surface.indices,
      box = this.surface.geometry.boundingBox;
    const cx = body.center.x,
      cz = body.center.z;
    this.span = Math.max(
      0.22,
      (box.max.x - box.min.x) * 2 + 0.04,
      (box.max.z - box.min.z) * 2 + 0.04,
      box.max.y * Math.max(Math.abs(D[0] / D[1]), Math.abs(D[2] / D[1])) * 2 + 0.12,
    );
    const projectedX = cx - (body.center.y * D[0]) / D[1],
      projectedZ = cz - (body.center.y * D[2]) / D[1];
    this.origin.set((cx + projectedX) / 2 - this.span / 2, (cz + projectedZ) / 2 - this.span / 2);
    for (let t = 0; t < ix.length; t += 3) {
      const vertices = [ix[t] * 3, ix[t + 1] * 3, ix[t + 2] * 3];
      const projected = vertices.map((i) => [
        p[i] - (p[i + 1] * D[0]) / D[1],
        p[i + 2] - (p[i + 1] * D[2]) / D[1],
      ]);
      ...
      if (height < 0.016)
        this.rasterTriangle(
          ...vertices.map((i) => [p[i], p[i + 2]]),
          this.contact,
          Math.exp(-height / 0.0028),
        );
    }
    this.blur(this.shadow);
    this.blur(this.contact);
    for (let i = 0; i < this.size * this.size; i++) {
      this.shadowBytes[i * 4] = Math.round(this.shadow[i] * 255);
      this.shadowBytes[i * 4 + 1] = Math.round(this.contact[i] * 255);
      this.shadowBytes[i * 4 + 2] = 0;
      this.shadowBytes[i * 4 + 3] = 255;
    }
  }
}
...
class RefractiveLightField {
  constructor(surface, lightDirection, sigma) {
    ...
    this.span = 0.22;
    this.origin = new THREE.Vector2();
    this.originNode = uniform(this.origin);
    this.spanNode = uniform(this.span);
    ...
    this.shadowSpan = 0.22;
    this.shadowSpanNode = uniform(this.shadowSpan);
    this.shadowOrigin = new THREE.Vector2();
    this.shadowOriginNode = uniform(this.shadowOrigin);
    this.contactOrigin = new THREE.Vector2();
    this.contactOriginNode = uniform(this.contactOrigin);
    this.shadowBytes = new Uint8Array(SHADOW_SIZE * SHADOW_SIZE * 4);
    this.shadowTexture = new THREE.DataTexture(
      this.shadowBytes,
      SHADOW_SIZE,
      SHADOW_SIZE,
      THREE.RGBAFormat,
    ...
    this.shadowTexture.colorSpace = THREE.NoColorSpace;
    ...
    this.lightRight = new THREE.Vector3(1, 0, 0);
    this.lightUp = new THREE.Vector3(0, 0, 1);
    this.lightExtent = 0.22;
    this.lightCameraPositionNode = uniform(this.lightCameraPosition);
    this.lightRightNode = uniform(this.lightRight);
    this.lightUpNode = uniform(this.lightUp);
    this.lightDirectionNode = uniform(this.lightDirection);
    this.lightExtentNode = uniform(this.lightExtent);
    this.receiverOriginNode = uniform(this.origin);
    this.receiverSpanNode = uniform(this.span);

    const maxId = surface.bindingIds.reduce((m, id) => Math.max(m, id), 0),
      nodeCount = maxId + 1;
    ...
    if (!geometry.getAttribute('cageIds'))
      geometry.setAttribute('cageIds', new THREE.BufferAttribute(surface.bindingIds, 4));
    if (!geometry.getAttribute('cageWeights'))
      geometry.setAttribute(
        'cageWeights',
        new THREE.BufferAttribute(Float32Array.from(surface.bindingWeights), 4),
      );
    const ids = attribute('cageIds'),
      weights = attribute('cageWeights');
    const deformed = Fn(() =>
      this.cageNode
        .element(ids.x)
        .xyz.mul(weights.x)
    ...
    frontMaterial.positionNode = deformed;
    frontMaterial.colorNode = vec4(depth, depth, depth, 1);
    frontMaterial.toneMapped = false;
    const backMaterial = new THREE.MeshBasicNodeMaterial({ side: THREE.BackSide });
    backMaterial.positionNode = deformed;
    backMaterial.colorNode = vec4(depth, depth, depth, 1);
    backMaterial.toneMapped = false;
    this.frontScene = new THREE.Scene();
    this.frontScene.background = new THREE.Color(0x000000);
    this.backScene = new THREE.Scene();
    this.backScene.background = new THREE.Color(0x000000);
    const frontMesh = new THREE.Mesh(geometry, frontMaterial),
      backMesh = new THREE.Mesh(geometry, backMaterial);
    frontMesh.frustumCulled = false;
    backMesh.frustumCulled = false;
    this.frontScene.add(frontMesh);
    this.backScene.add(backMesh);

    const reconstruct = (uvNode, depthNode) =>
      this.lightCameraPositionNode
        .add(this.lightRightNode.mul(uvNode.x.sub(0.5).mul(this.lightExtentNode)))
        .add(this.lightUpNode.mul(uvNode.y.sub(0.5).mul(this.lightExtentNode)))
    ...
    const normalFromDepth = (depthTexture, uvNode, centerDepth, rayDirection) => {
      const leftUV = vec2(uvNode.x.sub(texel), uvNode.y),
        rightUV = vec2(uvNode.x.add(texel), uvNode.y);
      const downUV = vec2(uvNode.x, uvNode.y.sub(texel)),
        upUV = vec2(uvNode.x, uvNode.y.add(texel));
      const dlRaw = texture(depthTexture, leftUV).r,
        drRaw = texture(depthTexture, rightUV).r;
      const ddRaw = texture(depthTexture, downUV).r,
        duRaw = texture(depthTexture, upUV).r;
      const dl = select(dlRaw.greaterThan(1e-5), dlRaw, centerDepth),
        dr = select(drRaw.greaterThan(1e-5), drRaw, centerDepth);
      const dd = select(ddRaw.greaterThan(1e-5), ddRaw, centerDepth),
        du = select(duRaw.greaterThan(1e-5), duRaw, centerDepth);
      const raw = cross(
        reconstruct(rightUV, dr).sub(reconstruct(leftUV, dl)),
        reconstruct(upUV, du).sub(reconstruct(downUV, dd)),
      );
      const lengthSq = raw.dot(raw);
      let n = select(
        lengthSq.greaterThan(1e-12),
        raw.div(lengthSq.max(1e-12).sqrt()),
        rayDirection.negate(),
      );
    ...
    const refractNode = (direction, normal, n1, n2) => {
      const eta = float(n1 / n2),
        cosine = direction.dot(normal).negate().clamp(0, 1);
    ...
    const entryNormal = normalFromDepth(
      this.frontTarget.texture,
      sourceUV,
      frontDepth,
      this.lightDirectionNode,
    );
    const entry = refractNode(this.lightDirectionNode, entryNormal, 1, IOR);
    const insideCos = entry.direction.dot(this.lightDirectionNode).max(0.12);
    const projectUV = (point) => {
      const offset = point.sub(this.lightCameraPositionNode);
      return vec2(offset.dot(this.lightRightNode), offset.dot(this.lightUpNode))
        .div(this.lightExtentNode)
        .add(0.5);
    };
    ...
    const valid = frontDepth
      .greaterThan(1e-5)
      .and(entry.valid)
      .and(backDepth.greaterThan(frontDepth.add(2e-5)))
      .and(exitUV.x.greaterThan(0))
      .and(exitUV.x.lessThan(1))
      .and(exitUV.y.greaterThan(0))
      .and(exitUV.y.lessThan(1))
      .and(exitDepth.greaterThan(frontDepth))
      .and(exit.valid)
      .and(exit.direction.y.lessThan(-1e-5))
    ...
    const dx = sourceUV.dFdx(),
      dy = sourceUV.dFdy();
    const horizontalFluxCorrection = 1 / Math.max(0.15, Math.abs(this.lightDirection.y));
    const inverseJacobian = dx.x
      .mul(dy.y)
      .sub(dx.y.mul(dy.x))
      .abs()
      .mul(CAUSTIC_SIZE * CAUSTIC_SIZE * horizontalFluxCorrection)
      .mul(
        this.lightExtentNode
          .mul(this.lightExtentNode)
          .div(this.receiverSpanNode.mul(this.receiverSpanNode)),
      );
    ...
    const schlick = (cosine) =>
      float(1).sub(float(r0).add(float(1 - r0).mul(float(1).sub(cosine).pow(5))));
    ...
    const transmittedEnergy = absorption
      .mul(transmission)
      .mul(select(valid, 1, 0))
      .toVarying();
    causticMaterial.colorNode = vec4(transmittedEnergy.mul(focus), 1);
    causticMaterial.depthTest = false;
    causticMaterial.depthWrite = false;
    causticMaterial.blending = THREE.AdditiveBlending;
    causticMaterial.toneMapped = false;
    this.causticScene = new THREE.Scene();
    this.causticScene.background = new THREE.Color(0x000000);
    const causticMesh = new THREE.Mesh(grid, causticMaterial);
    causticMesh.frustumCulled = false;
    this.causticScene.add(causticMesh);
    this.causticCamera = new THREE.OrthographicCamera(-1, 1, 1, -1, 0.1, 2);
    this.causticCamera.position.z = 1;

    const blurMaterial = new THREE.MeshBasicNodeMaterial();
    const uv = screenUV,
      du = float(1 / CAUSTIC_SIZE),
      raw = this.rawCausticTarget.texture;
    const blur = texture(raw, uv)
      .rgb.mul(0.28)
      .add(texture(raw, uv.add(vec2(du, 0))).rgb.mul(0.12))
      .add(texture(raw, uv.sub(vec2(du, 0))).rgb.mul(0.12))
      .add(texture(raw, uv.add(vec2(0, du))).rgb.mul(0.12))
      .add(texture(raw, uv.sub(vec2(0, du))).rgb.mul(0.12))
      .add(texture(raw, uv.add(vec2(du, du))).rgb.mul(0.06))
      .add(texture(raw, uv.add(vec2(du.negate(), du))).rgb.mul(0.06))
      .add(texture(raw, uv.add(vec2(du, du.negate()))).rgb.mul(0.06))
      .add(texture(raw, uv.sub(vec2(du, du))).rgb.mul(0.06));
    blurMaterial.colorNode = vec4(blur, 1);
  ...
  setAbsorption(sigma) {
    this.absorptionNode.value.set(-sigma[0], -sigma[1], -sigma[2]);
    this.absorptionDirty = true;
  }
  /** Table attenuation (cast shadow + contact) at a world XZ, exactly as the table shader applies it. */
  groundAttenuation(xz, windowFraction) {
    const field = (origin, pick) => {
      const uv = xz.sub(origin).div(this.shadowSpanNode);
      const inside = float(
        uv.x.greaterThan(0).and(uv.x.lessThan(1)).and(uv.y.greaterThan(0)).and(uv.y.lessThan(1)),
      );
      return pick(texture(this.shadowTexture, uv)).mul(inside);
    };
    const a = float(1)
      .sub(field(this.shadowOriginNode, (t) => t.r).mul(windowFraction))
      .mul(float(1).sub(field(this.contactOriginNode, (t) => t.g).mul(0.4)));
    // Shadow on warm wood is lit by sky, not the window, so it cools as it deepens rather than
    // sliding straight to grey. A colour, so the jelly divides the tint back out with the rest.
    return vec3(a).mul(a.oneMinus().mix(vec3(1), vec3(0.9, 0.95, 1.05)));
  }
  update(renderer, body, force = false) {
    if (
      !force &&
      !this.absorptionDirty &&
      this.lastRevision === body.surfaceRevision &&
      this.lastCenter.distanceToSquared(body.center) < 1e-14
    )
      return;
    this.lastRevision = body.surfaceRevision;
    this.lastCenter.copy(body.center);
    const x = body.x;
    for (let i = 0, j = 0; i < x.length; i += 3, j += 4) {
      this.cagePacked[j] = x[i];
      this.cagePacked[j + 1] = x[i + 1];
      this.cagePacked[j + 2] = x[i + 2];
      this.cagePacked[j + 3] = 0;
    }
    this.cageAttribute.needsUpdate = true;

    const D = this.lightDirection,
      box = body.surface.geometry.boundingBox,
      c = body.center;
    this.span = Math.max(
      0.22,
      (box.max.x - box.min.x) * 2 + 0.04,
      (box.max.z - box.min.z) * 2 + 0.04,
      box.max.y * Math.max(Math.abs(D.x / D.y), Math.abs(D.z / D.y)) * 2 + 0.12,
    );
    this.spanNode.value = this.span;
    this.receiverSpanNode.value = this.span;
    const projectedX = c.x - (c.y * D.x) / D.y,
      projectedZ = c.z - (c.y * D.z) / D.y;
    this.origin.set((c.x + projectedX) / 2 - this.span / 2, (c.z + projectedZ) / 2 - this.span / 2);
    this.originNode.value.copy(this.origin);
    this.receiverOriginNode.value.copy(this.origin);

    this.lightExtent = this.span;
    this.lightExtentNode.value = this.lightExtent;
    this.lightCamera.left = -this.lightExtent / 2;
    this.lightCamera.right = this.lightExtent / 2;
    this.lightCamera.top = this.lightExtent / 2;
    this.lightCamera.bottom = -this.lightExtent / 2;
    this.lightCamera.updateProjectionMatrix();
    this.lightCamera.position.copy(c).addScaledVector(D, -LIGHT_CAMERA_DISTANCE);
    if (Math.abs(D.z) > 0.96) this.lightCamera.up.set(0, 1, 0);
    else this.lightCamera.up.set(0, 0, 1);
    this.lightCamera.lookAt(c);
    this.lightCamera.updateMatrixWorld(true);
    this.lightCameraPosition.copy(this.lightCamera.position);
    this.lightCameraPositionNode.value.copy(this.lightCameraPosition);
    const e = this.lightCamera.matrixWorld.elements;
    this.lightRight.set(e[0], e[1], e[2]);
    this.lightUp.set(e[4], e[5], e[6]);
    this.lightRightNode.value.copy(this.lightRight);
    this.lightUpNode.value.copy(this.lightUp);

    renderer.setRenderTarget(this.frontTarget);
    renderer.render(this.frontScene, this.lightCamera);
    renderer.setRenderTarget(this.backTarget);
    renderer.render(this.backScene, this.lightCamera);
    renderer.setRenderTarget(this.rawCausticTarget);
  ...
  dispose() {
    ...
    this.rawCausticTarget.dispose();
    this.causticTarget.dispose();
    this.shadowTexture.dispose();
    this.resources.frontMaterial.dispose();
    this.resources.backMaterial.dispose();
    this.resources.grid.dispose();
    this.resources.causticMaterial.dispose();
    this.resources.blurMaterial.dispose();
  }
}
